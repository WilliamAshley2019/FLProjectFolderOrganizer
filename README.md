...
two more hex values are there however I am not sure if they are knob positionrelated I think they likely reference some other value like maybe peak and crest of lfo or something.
But it seems tied to the same 4 bytes for each knob section. Don't quite understand what the other two values are.
└──────────────────────────────────────┴──────────────────────────────────────────────┘
```
...
3. I need to fixure out a format the automations can be exported as, or create a export file type unique to automations so they can be migrated for other purposes
   ...
   b.) the current edit and "modify" automation data may make editing automations in flp "quick" rather than editing them all by hand youc an just modify them by entering the automation specific details. This may be easier for fine automation types. 
