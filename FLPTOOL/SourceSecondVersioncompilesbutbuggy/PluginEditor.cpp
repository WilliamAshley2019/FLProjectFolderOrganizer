...
{
    // 1. Initialize the unique_ptr to keep the chooser alive during the async operation
    fileChooser = std::make_unique<juce::FileChooser>(
        "Select an FL Studio Project...",
        juce::File::getSpecialLocation(juce::File::userDocumentsDirectory),
        "*.flp");

    // 2. Define flags for opening a file
    ...
    fileChooser->launchAsync(chooserFlags, [this](const juce::FileChooser& chooser)
        {
            ...
            // If the user cancelled the dialog, getResult() returns an empty File
            if (file == juce::File{})
                return;
