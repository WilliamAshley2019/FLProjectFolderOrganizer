    ...
    std::unique_ptr<Event> Event::read(juce::InputStream& in, EventID id, const FLVersion& version)
    {
        ...
        case EventID::Version:
            if (version >= FLVersion{ 11,5,0,0 })
                return std::make_unique<UnicodeEvent>(decodeUTF16LE(buffer.get(), dataSize));
            else
                return std::make_unique<AsciiEvent>(stripTrailingNul(juce::String::fromUTF8(reinterpret_cast<const char*>(buffer.get()), (int)dataSize)));
